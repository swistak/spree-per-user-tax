namespace :db do
  ...
  task :bootstrap  => :environment do
    ...
    Dir.glob(File.join(PerUserTaxExtension.root, "db", 'sample', '*.{yml,csv}')).each do |fixture_file|
      Fixtures.create_fixtures("#{PerUserTaxExtension.root}/db/sample", File.basename(fixture_file, '.*'))
    end
  end
end
